...
const { postFilm, getFilms, getFilmsByID, deleteFilmsByID } = require("../../db");
...
module.exports = (app) => {
    ...
    app.get(`${API_PATH}/films/:id`, async (req, res)=>{
        ...
        const resp = await getFilmsByID(id);
        return res.json(resp);
    });
    ...
    app.delete(`${API_PATH}/films/:id`, async (req, res)=>{
        const id = req.params.id;
        if(id){
            const resp = await deleteFilmsByID(id);
            return res.json(resp);
        } 
        res.status(400).send({ reason: "No film deleted." });
    });
}